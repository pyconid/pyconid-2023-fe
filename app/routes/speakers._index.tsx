...
export default function Route() {
  ...
  return (
    <Layout>
      ...
      <h1 className="mb-12 text-center font-brand text-5xl font-semibold text-primary lg:text-6xl">
        Keynote Speakers
      </h1>
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        <div className="mt-2 grid gap-4 md:grid-cols-2 md:gap-10 lg:grid-cols-3">
          {keynoteTalks.map((keynoteTalk, i) => (
            <SpeakerCard
              id={String(keynoteTalk.user?.id)}
              key={keynoteTalk.id}
              firstName={String(keynoteTalk.user?.firstName)}
              lastName={String(keynoteTalk.user?.lastName)}
              index={i}
              avatar={String(keynoteTalk.user?.avatar)}
            >
              {keynoteTalk.title}
            </SpeakerCard>
          ))}
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        <div className="mt-2 grid gap-4 md:grid-cols-2 md:gap-10 lg:grid-cols-4">
          {speakers.map((speaker, i) => (
            <SpeakerCard
              id={String(speaker.user?.id)}
              key={speaker.id}
              firstName={String(speaker.user?.firstName)}
              lastName={String(speaker.user?.lastName)}
              index={i}
              avatar={String(speaker.user?.avatar)}
            >
              {speaker.title}
            </SpeakerCard>
          ))}
